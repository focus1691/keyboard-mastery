...
class Keyboard extends Phaser.GameObjects.Container {
  constructor(scene, config) {
    ...
    let x = WIDTH / 2 - (KEY_W + KEY_X_SPACE) * KEY_SCALE_FACTOR * 4 - ((KEY_W + KEY_X_SPACE) * KEY_SCALE_FACTOR) / 2;
    ...
    for (let i = 0; i < KEYS.length; i++) {
      ...
      if (i === 0) {
        x = WIDTH / 2 - (KEY_W + KEY_X_SPACE) * KEY_SCALE_FACTOR * 4;
      } else if (i === 1) {
        x = WIDTH / 2 - (KEY_W + KEY_X_SPACE) * KEY_SCALE_FACTOR * 3;
      }
